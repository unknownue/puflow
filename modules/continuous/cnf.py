...
class CNF(nn.Module):
    ...
    def forward(self, x, context=None, logpx=None, integration_times=None, reverse=False, upratio=4):
        if logpx is None:
        ...
        if self.conditional:
            ...
            states = (x, _logpx)
            atol = self.atol # [self.atol] * 2
            rtol = self.rtol # [self.rtol] * 2
        ...
        if reverse:
            self.odefunc.before_odeint_inverse(upratio)
        else:
            self.odefunc.before_odeint_forward(e=None)
