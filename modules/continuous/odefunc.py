...
def divergence_approx(f, y, e=None, upratio=1):
    if y.shape != e.shape:
        e = torch.repeat_interleave(e, upratio, dim=1)
...
class ODEfunc(nn.Module):
    ...
    def before_odeint_forward(self, e=None):
        self._e = e
        self._num_evals.fill_(0)

    def before_odeint_inverse(self, upratio=4):
        self.upratio = upratio

    def forward(self, t, states):
        y = states[0]  # [B, N, D]
        t = torch.ones((y.shape[0], y.shape[1], 1)).to(y) * t.clone().detach().requires_grad_(True).type_as(y)
        ...
        with torch.set_grad_enabled(True):
            if len(states) == 3:  # conditional CNF
                ...
                tc = torch.cat([t, c], dim=-1)  # [B, N, C + 1]
                dy = self.diffeq(tc, y)

                divergence = self.divergence_fn(dy, y, e=self._e, upratio=self.upratio).unsqueeze(-1)
                return dy, -divergence, torch.zeros_like(c).requires_grad_(True)
            elif len(states) == 2:  # unconditional CNF
                dy = self.diffeq(t, y)
                divergence = self.divergence_fn(dy, y, e=self._e, upratio=self.upratio).view(-1, 1)
