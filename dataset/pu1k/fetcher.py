...
from dataset.pu1k import point_operation
...
def load_h5_data(h5_filename='', opts=None, skip_rate=1, use_randominput=True):
    ...
    if use_randominput:
        ...
        with h5py.File(h5_filename, 'r') as f:
            input = f['poisson_%d' % num_4X_point][:].astype(np.float32)
            gt = f['poisson_%d' % num_out_point][:].astype(np.float32)
    else:
        logging.info("Do not use random input")
        with h5py.File(h5_filename, 'r') as f:
            input = f['poisson_%d' % num_point][:].astype(np.float32)
            gt = f['poisson_%d' % num_out_point][:].astype(np.float32)
    ...
    gt = gt[::skip_rate]
    data_radius = data_radius[::skip_rate].astype(np.float32)
...
class Fetcher(threading.Thread):
    def __init__(self, opts, num_batches=None):
        ...
        while not self.stopped:
            ...
            for batch_idx in range(self.num_batches):
                ...
                if self.use_random_input:
                    new_batch_input = np.zeros((self.batch_size, self.patch_num_point, batch_input_data.shape[2]), dtype=np.float32)
                ...
                if self.opts.augment:
                    ...
                    batch_input_data, batch_data_gt, scales = point_operation.random_scale_point_cloud_and_gt(batch_input_data, batch_data_gt, scale_low=0.8, scale_high=1.2)
