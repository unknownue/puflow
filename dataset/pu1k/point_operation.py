...
def rotate_point_cloud_and_gt(batch_data, batch_gt=None, z_rotated=False):
    ...
    angles = np.random.uniform(size=(batch_size, 3)).astype(np.float32) * 2 * np.pi
...
def jitter_perturbation_point_cloud(batch_data, sigma=0.005, clip=0.02):
    ...
    """
    assert (clip > 0)
    jittered_data = np.clip(sigma * np.random.randn(*batch_data.shape).astype(np.float32), -1 * clip, clip)
    jittered_data[:, :, 3:] = 0
...
def random_scale_point_cloud_and_gt(batch_data, batch_gt=None, scale_low=0.5, scale_high=2):
    ...
    """
    B, N, C = batch_data.shape
    scales = np.random.uniform(scale_low, scale_high, (B, 1, 1)).astype(np.float32)
